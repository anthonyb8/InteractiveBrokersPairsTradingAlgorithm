...
def get_cointegrated_pairs(prices):
    ...
    for sym_1 in prices.keys():
        ...
        for sym_2 in prices.keys():
            if sym_2 != sym_1:
                ...
                # Get close prices
                series_1 = list(prices[sym_1].Close)
                series_2 = list(prices[sym_2].Close)
               
                if len(series_1) > len(series_2):
                    series_1 = series_1[:-(len(series_1) - len(series_2))]
                elif len(series_1) < len(series_2):
                    series_2 = series_2[:-(len(series_2) - len(series_1))]
    ...
    df_coint.to_csv("3_cointegrated_pairs.csv", index = False)
    return df_coint
