from func_position_calls import get_open_positions, get_open_orders
...
def check_order(ticker, order_id, remaining_capital, direction="Long"):
    ...
    # Get current orderbook/ streamsnapshot()
    orderbook = ws_public.fetch(f"orderBookL2_25.{ticker}")

    # Get latest price / get_trade_details()
    mid_price, _, _ = get_trade_details(orderbook)

    # Get active order details /  get_open_orders()
